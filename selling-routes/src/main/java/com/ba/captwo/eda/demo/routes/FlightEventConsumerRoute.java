...
public class FlightEventConsumerRoute extends RouteBuilder {
    ...
    @Value("${redis.route.uri}") private String redis_route_uri;
    ...
    public void configure() throws Exception {
        ...
        from(input_uri).routeId("CXFRS-Consume").startupOrder(2)        .id("CXFRS-Consume : Receive Request")
                ...
                .to(redis_route_uri)                                    .id("CXFRS-Consume : Send Message to Redis")
                .setBody(constant("OK"))                                .id("CXFRS-Consume : Respond to Client")
                .end();

        // > JMS Destination >

        from(redis_route_uri).routeId("SendToRedis").startupOrder(1)                     .id("SendToRedis")
